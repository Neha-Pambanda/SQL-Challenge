...
SELECT sa.salesman_id, sa.name, COUNT(*) AS total_number_of_cars_sold
FROM sales AS s
INNER JOIN cars AS c
ON s.car_id = c.car_id
INNER JOIN salespersons AS sa
ON s.salesman_id = sa.salesman_id
GROUP BY sa.salesman_id;

--3. What is the total revenue generated by each salesperson?;
SELECT sa.salesman_id, sa.name, SUM(c.cost_$) AS total_revenue
FROM sales AS s
INNER JOIN cars AS c
ON s.car_id = c.car_id
INNER JOIN salespersons AS sa
...
SELECT sa.name, c.car_id, c.make, c.type, c.style, c.cost_$
FROM sales AS s
INNER JOIN cars AS c
ON s.car_id = c.car_id
INNER JOIN salespersons AS sa
...
SELECT s.salesman_id, sa.name, sa.city, COUNT(*) AS total_sales
FROM sales AS s
JOIN salespersons AS sa
ON s.salesman_id = sa.salesman_id
WHERE YEAR(s.purchase_date) = 2023
GROUP BY s.salesman_id, sa.name, sa.city
ORDER BY total_sales DESC
LIMIT 1;

--10. What is the name and age of the salesperson who generated the highest revenue in the year 2022?;
SELECT sa.name, sa.age
FROM salespersons sa
JOIN sales s ON sa.salesman_id = s.salesman_id
JOIN cars c ON s.car_id = c.car_id
WHERE YEAR(s.purchase_date) = 2022
GROUP BY sa.salesman_id, sa.name, sa.age
